...
when_to_use: "[sasp-scoring] SASP衰老分泌表型评分：衰老细胞表达→SASP基因集→衰老评分→药物干预效果"
version: 1.0.0
author: MemOmics
license: MIT
...
# SASP Scoring (AUCell/UCell)

SASP gene set scoring + heatmap + group comparison

适用场景: aging

分析步骤:
  - SASP gene set definition: Literature + database genes
  - UCell/AUCell scoring: Single-cell SASP activity
  - Group comparison + viz: Aging vs control SASP diff

依赖包: Seurat, AUCell, scanpy, UCell, gseapy

难度: basic

触发提示: "Calculate SASP scores"
...
1. **SASP gene set definition**
   - Literature + database genes
   - Tool: `terminal`
2. **UCell/AUCell scoring**
   - Single-cell SASP activity
   - Tool: `terminal`
3. **Group comparison + viz**
   - Aging vs control SASP diff
   - Tool: `terminal`
...
| `r_packages` | Seurat, UCell, AUCell | |
| `python_packages` | scanpy, gseapy | |
| `steps` | SASP gene set definition -> UCell/AUCell scoring -> Group comparison + viz | |
