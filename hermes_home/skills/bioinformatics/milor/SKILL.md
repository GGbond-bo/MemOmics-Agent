...
when_to_use: "[milor] MiloR邻域差异丰度检验, 适用于多条件比较"
version: 1.0.0
author: MemOmics
license: MIT
...
MiloR邻域差异丰度检验, 适用于多条件比较

适用场景: 有分组的scRNA-seq, aging, disease, >5样本

分析步骤:
  - KNN graph + neighborhoods: MiloR from Seurat
  - Differential abundance test: testNhoods + spatial FDR
  - Visualization: Significant neighborhoods on UMAP

依赖包: edgeR, Seurat, milo-py, miloR
...
适用于: 有分组的scRNA-seq, aging, disease, >5样本

## Pipeline

1. **KNN graph + neighborhoods**
   - MiloR from Seurat
   - Tool: `terminal`
2. **Differential abundance test**
   - testNhoods + spatial FDR
   - Tool: `terminal`
3. **Visualization**
   - Significant neighborhoods on UMAP
   - Tool: `terminal`
...
| `r_packages` | miloR, Seurat, edgeR | |
| `python_packages` | milo-py | |
| `steps` | KNN graph + neighborhoods -> Differential abundance test -> Visualization | |
