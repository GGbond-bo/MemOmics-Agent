...
when_to_use: "[soupx-remove-background] SoupX背景RNA去除：原始+过滤矩阵→背景污染估计→表达校正→去污染矩阵"
version: 1.0.0
author: MemOmics
license: MIT
...
SoupX基于soup模型估计环境RNA。轻量级，CPU即可运行。适用于轻度到中度污染，或CellBender不可用时的替代。需要raw+filtered矩阵。参数自适应: 高污染组织→soupFraction上限0.3

## When to Use

需去除环境RNA但无GPU，或CellBender的替代/补充
...
1. 加载raw+filtered矩阵
2. estimateSoup()估计soup比例
3. autoEstCont()自动估计污染
4. adjustCounts()校正
5. 输出校正后矩阵
