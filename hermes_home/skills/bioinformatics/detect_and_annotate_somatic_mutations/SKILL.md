...
when_to_use: "[detect_and_annotate_somatic_mutations] 体细胞突变检测与注释：BAM/VCF→mutect2/strelka2→VEP/ANNOVAR注释→驱动突变→临床可操作性"
version: 1.0.0
author: MemOmics
license: MIT
...
Detects and annotates somatic mutations in tumor samples compared to matched normal samples using GATK Mutect2 for variant calling, GATK FilterMutectCalls for filtering, and SnpEff for functional annotation.

## When to Use

When you need detect and annotate somatic mutations analysis

## Parameters

| Parameter | Default | Notes |
|-----------|---------|-------|
| `tumor_bam` | [Required] Path to the tumor sample BAM file (str) | |
| `normal_bam` | [Required] Path to the matched normal sample BAM file (str) | |
| `reference_genome` | [Required] Path to the reference genome FASTA file (str) | |
| `output_prefix` | [Required] Prefix for output files (str) | |
| `snpeff_database` | [Optional] SnpEff database to use for annotation (default: GRCh38.105) | |
